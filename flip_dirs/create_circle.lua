...
delete_hcircle = function(x, y, z, radius)
    for first = -radius, radius do
        for second = -radius, radius do
            if (first * first) + (second * second) <=
                ((radius * radius) + (radius * 1.2)) then
                minetest.set_node({x = x + first, y = y + 0, z = z + second},
                                  {name = "air"})
                local node = minetest.get_meta(
                                 {x = x + first, y = y + 0, z = z + second})
...
delete_vcircle = function(x, y, z, radius)
    for first = -radius, radius do
        for second = -radius, radius do
            if (first * first) + (second * second) <=
                ((radius * radius) + (radius * 1.2)) then
                minetest.set_node({x = x + first, y = y + second, z = z + 0},
                                  {name = "air"})
                local node = minetest.get_meta(
                                 {x = x + first, y = y + second, z = z + 0})
                node:set_string("origin", nil)
            end
        end
    end
...
create_vcircle = function(x, y, z, radius)
    local origin = {x = x, y = y, z = z, r = radius, o = "v"}
    local empty_nodes = {}
    local origin = minetest.serialize(origin)
    for first = -radius, radius do
        for second = -radius, radius do
            if (first * first) + (second * second) <=
                ((radius * radius) + (radius * 1.2)) then
                minetest.set_node({x = x + first, y = y + second, z = z},
                                  {name = "cdmod:platform"})
                local node = minetest.get_meta(
                                 {x = x + first, y = y + second, z = z})
                table.insert(empty_nodes, {x = first, y = second})
                node:set_string("origin", origin)
            end
        end
