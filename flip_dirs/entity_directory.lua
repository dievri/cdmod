...
minetest.register_entity("cdmod:directory", {
    ...
    -- find glass in radius of 6
            ...
            local node = minetest.get_meta({x = node_pos.x, y = node_pos.y, z = node_pos.z})
            local corner = node:get_string("corner")
            local corner_pos = minetest.deserialize(corner)
            local corner_node = minetest.get_meta({x = corner_pos.x, y = corner_pos.y, z = corner_pos.z})
            ...
            local result = create_platform(posx, new_level, posz, platform_size, "h", content, host_info)
